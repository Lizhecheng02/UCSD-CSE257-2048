...
class GameRunner:
    ...
    def print_matrix(self):
        ...
        for i in range(0, self.game.board_size):
            for j in range(0, self.game.board_size):
                ...
                pygame.draw.rect(self.surface, color, (ul_x_px, ul_y_px, tile_size, tile_size))
                pygame.draw.rect(self.surface, BLACK, (ul_x_px, ul_y_px, tile_size, tile_size), 2)
    ...
    def draw_label_hl(self, pos, label, padding=PADDING, bg=WHITE, wd=2, border=True):
        ...
        for color, width in specs:
            pygame.draw.rect(self.surface, color, (pos[0] - padding, pos[1] - padding, label.get_width() + padding * 2, label.get_height() + padding * 2), width)
